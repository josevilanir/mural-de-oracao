...
export default async function AdminPage() {
  ...
  const [prayers, pendingGroups, activeGroups, removalRequests] =
    await Promise.all([
          ...
          {pendingGroups.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {pendingGroups.map((group) => (
                <div
                  key={group.id}
                  className="bg-card rounded-lg border border-gray-200 dark:border-gray-med/30 shadow-sm p-4 flex items-start justify-between gap-4"
                >
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500">
                      Líder: {group.leader.name} · {group._count.members}{" "}
                      membro(s)
                    </p>
