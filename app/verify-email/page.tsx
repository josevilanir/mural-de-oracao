...
export default async function VerifyEmailPage({ searchParams }: Props) {
  ...
  if (!result.success) {
    return (
      <Result
        ok={false}
        message={result.error ?? "Link inválido ou expirado."}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center p-4">
      <div className="bg-card rounded-xl shadow-sm p-8 w-full max-w-md text-center flex flex-col items-center gap-4">
        ...
        <p
          className={`text-sm font-medium ${ok ? "text-navy" : "text-red-500"}`}
        >
          {message}
        </p>
        <Link
          href={ok ? "/" : "/login"}
          className="text-blue-main hover:underline text-sm font-medium"
        >
